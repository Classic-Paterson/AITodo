...
function writePreamble(outputFilePath, preambleFilePath) {
  if (preambleFilePath) {
    ...
    appendFileSync(
      outputFilePath,
      'The following text is a Git repository with code. The structure of the text are sections that begin with ----, followed by a single line containing the file path and file name, followed by a variable amount of lines containing the file contents. The text representing the Git repository ends when the symbols --END-- are encountered. Before the --END-- symbol, only instructions using the aforementioned Git repository as context should be included.\n'
    );
  }
}
